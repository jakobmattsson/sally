...
.put('/accounts/#{account1}', { name: 'new name' })
.res('Updating account as admin', (data) -> data.should.include { name: 'new name' })
.auth('ua1', 'p')
.get('/accounts')
.res('Reading accounts as account admin', (data) -> data.should.have.lengthOf 1)
.put('/accounts/#{account1}', { name: 'new name again' })
.res('Updating account as account admin', (data) -> data.should.include { name: 'new name again' })
.post('/accounts')
.err(401, 'unauthed')
.auth('ua2', 'p')
.get('/accounts')
